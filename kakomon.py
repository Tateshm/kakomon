...
def main():
    ...
    layout = [
        ...
        [sg.Checkbox('すべてチェック', enable_events=True, key='chkall')],
        exam_list_parts,
    ...
    while True:
        ...
        # Exit the program when top-right X button is pressed, or 'QUIT' button is pressed
        if event == sg.WIN_CLOSED or event == 'quit':
            break
        # Check all checkboxes when check-all button is pressed
        elif event == 'chkall':
            for e in exams:
                k = e['key']
                window[k].update(value=values['chkall'])
